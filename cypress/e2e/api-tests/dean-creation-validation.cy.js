    ...
    cy.generateToken(loginData[0].username,loginData[0].password).then(
        ...
        (token)=>{
            ...
            cy.request({
              ...
              expect(response.body[response.body.length-1]).to.have.property("username");//en sonki elementin username property sinin verification
              ...
              expect(
                response.body.some((user)=>user.ssn===userData[0].ssn)
              ).to.be.true
